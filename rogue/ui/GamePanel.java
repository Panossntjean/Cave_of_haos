    ...
    protected void paintComponent(Graphics g) {
        ...
        for (int x = 0; x < gameMap.getWidth(); x++) {
            for (int y = 0; y < gameMap.getHeight(); y++) {
                ...
                if (item != null) {
                    int ovalSize = tileSize / 4;
                    int offset = (tileSize - ovalSize) / 4;

                    if (vis == GameMap.VisibilityState.VISIBLE) {
                        g.setColor(Color.YELLOW); // bright yellow
                        g.fillOval(x * tileSize + offset, y * tileSize + offset, ovalSize, ovalSize);
                    } else if (vis == GameMap.VisibilityState.FOGGY) {
                        g.setColor(new Color(150, 150, 50)); // dull yellow / shadowed
                        g.fillOval(x * tileSize + offset, y * tileSize + offset, ovalSize, ovalSize);
                    }
        ...
        /*  // here we had the exit appear if all the enemies of the floor were killed but the logic of the game change since now we can spawn enemies as we move 
        if (enemies.isempty()) {
            //int[] exit = gameMap.getExit();
            g.setColor(Color.GREEN);
