...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // Force it to the foreground on stubborn window managers
        ...
        frame.setAutoRequestFocus(true); // (Java 9+)

        SwingUtilities.invokeLater(() -> {
            gamePanel.setFocusable(true);
            gamePanel.setRequestFocusEnabled(true);
            gamePanel.setFocusTraversalKeysEnabled(false); // so arrows aren't eaten by focus traversal
            gamePanel.requestFocusInWindow();  // primary
            gamePanel.grabFocus();             // extra nudge on some WMs
        });
        */
        player.setOnWin(() -> {
        ...
        SwingUtilities.invokeLater(() -> {
            frame.setAutoRequestFocus(true); // Java 9+
            frame.toFront();
            // Briefly toggle always-on-top to jump ahead of other windows, then revert
            frame.setAlwaysOnTop(true);
            frame.setAlwaysOnTop(false);
            // Make sure the panel still has the keyboard focus
            gamePanel.requestFocusInWindow();
        });

        // ---- Focus: give it directly to the GamePanel (not the frame) ----
        gamePanel.setFocusable(true);
        gamePanel.setRequestFocusEnabled(true);
        gamePanel.setFocusTraversalKeysEnabled(false); // so arrow keys aren't eaten

        // Run after the window is realized, so focus sticks
        SwingUtilities.invokeLater(() -> gamePanel.requestFocusInWindow());

        // Optional: if the window loses focus and comes back, re-focus the panel
        ...
        // ---- Focus handling (aggressive but safe) ----
        gamePanel.setFocusable(true);
        gamePanel.setRequestFocusEnabled(true);
        gamePanel.setFocusTraversalKeysEnabled(false); // arrows won't be eaten
            
        // 1) Immediately bring to front & activate app
        frame.toFront();
        frame.requestFocus();
            
        // 2) Toggle always-on-top briefly to beat focus-stealing protection
        SwingUtilities.invokeLater(() -> {
            frame.setAlwaysOnTop(true);
            frame.setAlwaysOnTop(false);
            gamePanel.requestFocusInWindow();
        ...
        // 3) Retry a few times in case the OS still resists (Windows often does)
        new javax.swing.Timer(50, new ActionListener() {
            int tries = 0;
            @Override public void actionPerformed(ActionEvent e) {
                if (gamePanel.isFocusOwner() || tries > 10) {
        ...
        // 4) If window regains focus later, give it to the panel
        frame.addWindowFocusListener(new WindowAdapter() {
            @Override public void windowGainedFocus(WindowEvent e) {
                gamePanel.requestFocusInWindow();
            }
        ...
        gamePanel.addKeyListener(new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent e) {
                ...
                boolean[][] occupied = new boolean[currentMap.getWidth()][currentMap.getHeight()];
                occupied[player.getX()][player.getY()] = true;
                for (Enemy en : enemies) {
                    occupied[en.getX()][en.getY()] = true;
                }
                ...
                for (Enemy enemy : enemies) {                    
                    enemy.takeTurn(player.getX(), player.getY(), floorMap,occupied);
                ...
                if (gameWorld.isPlayerOnExit()){                    
                    ...
                    //currentMap = gameWorld.getCurrentMap();                     
                    if(floorUp){
                ...
                if (gameWorld.isPlayerOnEntrance()) {
                    ...
                    floorMap = generateFloorMap(gameWorld.getCurrentMap());
                    enemies = gameWorld.getCurrentEnemies();
                    if (floorDown) {
