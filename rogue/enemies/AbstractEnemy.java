...
public abstract class AbstractEnemy implements Enemy {
    ...
    public void takeTurn(int playerX, int playerY, boolean[][] floorMap) {
        ...
        if (distX > distY) {
            tryMove(x + dx, y, floorMap,playerX,playerY);
        } else if (distY > distX) {
            tryMove(x, y + dy, floorMap,playerX,playerY);
        } else {
            if (distX > 2 && distY > 2){ // don't step on player 
                if (random.nextBoolean()) {
                    if (!tryMove(x + dx, y, floorMap,playerX,playerY)) tryMove(x, y + dy, floorMap,playerX,playerY);
                } else {
                    if (!tryMove(x, y + dy, floorMap,playerX,playerY)) tryMove(x + dx, y, floorMap,playerX,playerY);
                }
            }
        }
    }

    protected boolean tryMove(int newX, int newY, boolean[][] floorMap,int playerX, int playerY) {
